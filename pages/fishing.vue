<template>
  <div class="bg-dark">
    <div v-for="(config, index) in contentStore.featuredProviders.pageConfigs" :key="index">
      <img v-if="config.category === 'fish'" :src="config.image" :alt="`Image for ${config.category}`"
      ...
      <div class="my-4 w-full h-auto boxshadow">
        ...
        <div class="hidden lg:block md:block 2xl:block">
          <div class="grid grid-cols-8 gap-2.5">
            <div class="bg-darklight rounded-md" v-for="(provider, index) in publicStore.public.game_providers?.fish"
              :key="index">
        ...
        <div class="relative mx-1">
          <swiper ref="mySwiper" class="visible md:invisible swiper-container"
            ...
            <SwiperSlide v-for="(provider, index) in publicStore.public.game_providers?.fish" :key="index"
            ...
            <div class="swiper-button-next" :class="{ 'missing': isAtEnd }"></div>
            <div class="swiper-button-prev" :class="{ 'missing': isAtBeginning }"></div>
          </Swiper>
        </div>
      </div>
      <div class="p-2">
...
const isAtBeginning = ref(true); // Left button hidden at start
const isAtEnd = ref(false); // Right button shown at start
...
const favoriteCodes = computed(() => {
  const favoritefishs = memberStore.profile.favorite?.fish;
  if (Array.isArray(favoritefishs)) {
    return favoritefishs.map(fav => fav.code);
  } else {
    return [];
  }
});
