<template>
  <div class="bg-dark">
    <div v-for="(config, index) in contentStore.featuredProviders.pageConfigs" :key="index">
      <img v-if="config.category === 'sports'" :src="config.image" :alt="`Image for ${config.category}`"
           class="h-full w-full object-cover align-middle overflow-clip" />
    </div>
    <div class="w-full py-10 container mx-auto">
      <ul
        class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 mx-2 sm:mx-4 md:mx-8 lg:mx-16 gap-3 lg:gap-5 md:gap-5">
        <li v-for="(provider, index) in providersWithStatus" :key="index" class="relative">
          <div v-if="provider.status !== 'maintenance'"
            ...
            <div class="relative border border-gray-700 text-coco">
              <div class="flex-col relative inline-flex">
...
let newWindow = null;
const pageSlider = computed(() => contentStore.pageSlider?.sport);

const pageSport = computed(() => {
  return pageSlider.value?.find(page => page.id === 'page-sport') || null;
});

const providersWithStatus = computed(() => {
  if (!publicStore.public.game_providers?.sports || !contentStore.featuredProviders.sports) {
    return [];
  }

  return contentStore.featuredProviders.sports.map(provider => {
    const matchingProvider = publicStore.public.game_providers.sports.find(p => p.code === provider.code);
    return {
      ...provider,
