<template>
  <div class="bg-dark">
    <div v-for="(config, index) in contentStore.featuredProviders.pageConfigs" :key="index">
      <img v-if="config.category === 'pvp'" :src="config.image" :alt="`Image for ${config.category}`"
           class="h-full w-full object-cover align-middle overflow-clip" />
    </div>
    <div class="w-full py-10 container mx-auto">
      <ul class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 mx-2 sm:mx-4 md:mx-8 lg:mx-16 gap-3.5">
...
let newWindow = null;
const pageSlider = computed(() => contentStore.pageSlider?.pvp);

const pagePvp = computed(() => {
  return pageSlider.value?.find(page => page.id === 'page-poker') || null;
});

const providersWithStatus = computed(() => {
  if (!publicStore.public.game_providers?.pvp || !contentStore.featuredProviders.pvp) {
    return [];
  }

  return contentStore.featuredProviders.pvp.map(provider => {
    const matchingProvider = publicStore.public.game_providers.pvp.find(p => p.code === provider.code);
    return {
      ...provider,
